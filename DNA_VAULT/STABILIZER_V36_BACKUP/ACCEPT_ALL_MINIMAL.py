...
# 3. エディタ最前面時はH8狙撃(1249, 531)のみ。F8は封印。
...
def is_user_operating(orig_pos, duration=0.5, check_interval=0.05):
    """指定時間内(duration)にマウスが動いたか、あるいはキー入力があったかを厳格にチェック"""
    ...
    for _ in range(steps):
        ...
        if abs(curr_pos[0] - orig_pos[0]) > 1 or abs(curr_pos[1] - orig_pos[1]) > 1:
            return True
        # ESC以外のキー入力も「師匠のご操作」として尊ぶ
        for i in range(0x01, 0xFF):
            if i == 0x1B: continue # ESCは除外（自害用）
...
def execute_ultimate_dual_snipe(is_foreground):
    ...
    # 【H8狙撃】
    if is_foreground:
        strike_ritual(1249, 531, "H8_STRIKE", use_f8=False)
        return
...
if __name__ == "__main__":
    ...
    while True:
        try:
            with open(r"C:\StockProject\sniper_heartbeat.txt", "w") as f: f.write(str(time.time()))
