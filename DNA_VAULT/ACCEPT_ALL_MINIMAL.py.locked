...
# --- 極小メモリ・24時間稼働 AcceptAll エンジン (v51-Heartbeat) ---
# 師匠の命により、メモリ消費を極限まで削ぎ落とし、
# 24時間・30秒周期で「完全無音・確実」にAccept Allを撃ち抜く専用機構。
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--revived":
        # 蘇生時には一度だけ目立たせる（師匠への報告）
        try: ctypes.windll.kernel32.SetConsoleTitleW("PHOENIX_SNIPER_REVIVED")
        except: pass
